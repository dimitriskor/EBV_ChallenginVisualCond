    ...
    for rec_name in recording_names:
        ...
        for idx, image in enumerate(images):
            ...
            if args.dataset == 'hold':
                if (np.max(distance[low:high] < 0.025) and ('zurich_city_06' in rec_name or 'zurich_city_11_a' in rec_name)):
                    # shutil.copyfile(images_prefix+rec_name+'/'+image, f'{data_im_prefix_test}/{rec_name}_0{image[7:]}')
                    cv2.imwrite(f'{data_im_prefix_test}/{rec_name}_0{image[7:]}', img)
                    is_in_test.append(True)
                else:
            ...
            if args.dataset == 'train':
                if (np.max(distance[low:high] < 0.025) and not ('zurich_city_06' in rec_name or 'zurich_city_11_a' in rec_name)):
                    # shutil.copyfile(images_prefix+rec_name+'/'+image, f'{data_im_prefix_test}/{rec_name}_0{image[7:]}')
                    cv2.imwrite(f'{data_im_prefix_test}/{rec_name}_0{image[7:]}', img)
                    is_in_test.append(True)
                else:
        ...
        for idx, target in enumerate(targets[:len(is_in_test)]):
            if not is_in_test[idx]:
                continue
