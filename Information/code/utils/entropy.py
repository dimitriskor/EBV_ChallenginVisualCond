...
def difference(image, levels=16):
    '''
        Calculates the 1st order entropy for the image per timestep
        Levels: the discrete levels/probability values
    ...
    '''
    image = image[image.shape[0]//5:-image.shape[0]//5, image.shape[1]//5:-image.shape[1]//5]
    # image_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # image_gray = np.repeat(image_gray[..., np.newaxis], 3, axis=-1)
    # diff = np.sum(image - image_gray)
    added_axis = np.sum(image, axis=-1)
    kernel = np.array([[0.25, 0.25],
                       [0.25, 0.25]])
    smoothed = block_reduce(added_axis, block_size=(25, 25), func=np.mean)


    diff_small = np.sum(smoothed < 10)# or added_axis[added_axis > 700]) 
    diff_big = np.sum(smoothed > 750)# or added_axis[added_axis > 700]) 
...
def partial_contrast(image, levels, delta = 0.0078, downsample = 1, ranges = None):
    ...
    image = image[image.shape[0]//4:-image.shape[0]//4, image.shape[1]//4:-image.shape[1]//4]/255
    ...
    def neighbours(image, k, ranges):
        ...
        # print(x_values_h)
        # sd
        c_matrix_h = np.sum((x_values_h - y_values_h) ** 2, axis=2)/3# *   (2*(((1 - np.sum((x_values_h-x_values_h_gray[:, :, np.newaxis]) ** 2, axis=2)) * (1 - np.sum((y_values_h-y_values_h_gray[:, :, np.newaxis]) ** 2, axis=2)) > 0.99) -0.5))
        c_matrix_v = np.sum((x_values_v - y_values_v) ** 2, axis=2)/3# *   (2*(((1 - np.sum((x_values_v-x_values_v_gray[:, :, np.newaxis]) ** 2, axis=2)) * (1 - np.sum((y_values_v-y_values_v_gray[:, :, np.newaxis]) ** 2, axis=2)) > 0.99) -0.5))
