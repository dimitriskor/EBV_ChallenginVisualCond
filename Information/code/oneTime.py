...
def identify_detections(target, output, conf_thre=0.5, nms_thre=0.7, bbox_per_cell=3):
    ...
    def preprocess(prediction, conf_thre=0.5, nms_thre=0.7, bbox_per_cell=3):
    ...
    return iou, pred_det, target_det
...
for epoch in range(1):
    for data, target, flag in tqdm(trainloader):
        ...
        output = model(data)
        iou, pred, targ = identify_detections(target[0], output[-1][0])
        frame = data[-1][0].to('cpu')
        frame = frame.permute(1,2,0)
        iou = torch.tensor(iou)
        if True:#(len(iou) == 1) and torch.sum((0.8 < iou)*(0.99 > iou)) >= 1:
            # for det in pred:
            ...
            #     color = (0, 0, 1)  # Red for bounding boxes
            ...
            for det in targ:
                ...
                # Define label text and color
                label = f"Class {class_id}: {confidence:.2f}"
                color = (1, 0, 0)  # Red for bounding boxes
                # Draw the bounding box and label
                frame = np.ascontiguousarray(frame, dtype=np.uint8)
