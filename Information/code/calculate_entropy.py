...
def calculate_entropy_frames(names, vis_type, info_type, entropy_fn, ranges=None):
    ...
    dataset = 'info_DDD20' if 'DDD20' in prefix else 'info_DSEC'
    for file in names:
        if 'zurich_city_03' in file or 'zurich_city_09' in file or 'zurich_city_10' in file or 'zurich_city_12' in file: # or 'zurich_city_01' in rec_name or 'zurich_city_02' in rec_name:
            continue 
        print(f'Calculating event entropy in file {file} of type {vis_type}')
        # filename = prefix + vis_type + '/' + file                                                   # Used when working on DDD20
        filename = prefix + file
...
def calculate_entropy(names, vis_tupe, information_type = None, ranges=None):
    ...
    if information_type == 'difference':
        entropy_fn = entropy.difference
...
ranges = [0, 1*d, 2*d, 4*d, 8*d, 16*d, 32*d, 64*d, 128*d, 256*d, 3]

calculate_entropy(names, '', 'difference', ranges = ranges)
